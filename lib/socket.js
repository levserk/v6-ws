...
Socket.prototype.send = function(data){
    ...
    try{
        ...
        util.log('error', 'Socket.sendMessage',this.id, e, data);
    }
};
...
Socket.prototype.onClose = function(code){
    if (global.logLevel > 2) util.log('log;', 'closing socket', this.id, ' ws.state:', this.ws.readyState);
    if (this.closed){
    ...
    this.closed = true;
    this.emit('disconnect', self.closeReason||'close_code: ' + code);
    this.leaveAll();
    this.clean();
};
