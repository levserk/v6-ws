...
Socket.prototype.bindEvents = function(){
    var self = this;
    self.ws.on('pong',function(){
        self.reseiveTime = (new Date).valueOf();
    });
...
Socket.prototype.ping = function(){
    this.sendTime = (new Date).valueOf();
    this.ws.ping(1);
...
Socket.prototype.onMessage = function(data){
    ...
    try{
        ...
        util.log('error;', 'Socket.onMessage', e, data);
        if (data.length > 0) this.emit('message', data);
    }
};
